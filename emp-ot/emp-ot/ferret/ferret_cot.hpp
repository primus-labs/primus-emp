...
#if !(__EMSCRIPTEN__ || THREADING)
		// todo: fix on multi-thread
...
void FerretCOT<T>::setup(std::string pre_file) {
    ...
    p_ot_pre_data.reset(ot_pre_data);
	...
	if(hasfile & hasfile2) {
        ...
        std::unique_ptr<block[]> p_pre_data_ini(pre_data_ini);
